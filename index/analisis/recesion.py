...
def analyze_recession(recession, data, **args):
    ...
    data_recession
    recession_period_string = "from " + dt.datetime.strftime(recession[0].date(),
                                                             "%Y-%m-%d") + "; to " + dt.datetime.strftime(
        recession[1].date(), "%Y-%m-%d")
    columns_drop = [i for i in data_recession.isna().sum().index if
                    data_recession.isna().sum()[i] > data_recession.shape[0] / 10]
    logger.info(
        "recesion_analisis: {}, stocks to drop {}, stocks number (plus index) {}".format(recession, columns_drop,
                                                                                         data_recession.shape[1] - len(
                                                                                             columns_drop)))
...
if __name__ == "__main__":
    ...
    for recession in periods:
        changes, recession_period_string = analyze_recession(recession, data, **{"names": names, "sectors": sectors,
