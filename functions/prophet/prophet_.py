...
def report_cross_validation(forecast_full, forecast, df_cv, df_metrics, feature, model):
    ...
    for i, ind in enumerate(indices):
        ...
        if i < len(indices) - 1:
            ...
            logger.info('prophet_: METRICAS desde fecha {} hasta fecha: {}, {}'.format(ind, forecast_full.ds.values[-1], metrics))
    # metricas en cross validation y validation test
    plot_cross_validation_metric(df_cv, metric='mape')
    plot_cross_validation_metric(df_cv, metric='smape')
    plot_cross_validation_metric(df_cv, metric='rmse')
    plt.show()
    logger.info("prophet_: METRICAS medio en cross validation: rmse={}, smape={}, mape={},".format(df_metrics["rmse"].mean(),
