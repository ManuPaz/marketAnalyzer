...
def modify_like_clause(section,session,name):
        ...
        for filter in params["adhoc_filters"]:
            if filter["subject"]=="event":
                ...
                if name in config["superset"]["filters_replace"].keys() and event in config["superset"]["filters_replace"][name].keys():
                    filter["comparator"]=filter["comparator"].replace(event,config["superset"]["filters_replace"][name][event])
...
if __name__ == "__main__":
    ...
    for section in charts:
          ...
          if modifying_like_clause:
              ...
              if name is not None:
                modify_like_clause(section.copy(), session,name)
          if add_charts:
