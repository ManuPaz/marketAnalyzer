...
def get_fundamental_multiples_columns(exchange, stock, freq="M", columnas=[], bd=None):
    ...
    try:
        ...
        logger.error("database_functions:get_fundamental_multiples_columns- {}_{}, error {}".format(exchange,stock,e))
        return None
...
def get_multiple_macro_data(pais, shift=0, bd=None):
    ...
    for evento in eventos_macro:
        serie = get_series_activos_diferentes_de_acciones(pais, evento, "macro", "M",
                                                          evento.replace("%", "").replace("()", "").strip(), bd)
...
def obtener_multiples_series(tipo, freq, bd=None, *array):
    """
    ...
    if tipo == "index":
        ...
        for nombre in array:
            try:
                ...
                logger.error("database_functios->obtener_multiples_series->precios: {}".format(e))
...
def obtener_fundamenal_varios_stocks_multiples_columns(freq, array1=None, array2=None, diccionario=None, bd=None):
    ...
    if array1 is not None:
        for stock in array1:
            ...
            if serie is not None:
                serie["stock"] = stock
                serie.set_index("stock", inplace=True,append=True)
                serie = serie.unstack()
                series.append(serie)

            serie = get_prize_or_fundamenal(exchange, stock, freq=freq, type="precios", columna="adjusted_close",
                                            col_name="adjusted_close", bd=bd)
            if serie is not None:
                serie["stock"] = stock
                serie.set_index("stock", inplace=True, append=True)
                serie = serie.unstack()
...
if __name__ == "__main__":
    ...
    data = get_series_activos_diferentes_de_acciones("united kingdom",
                                                     config["macro"]["eventos_fundamentales"]["united kingdom"][6],
                                                     "macro",
                                                     freq='M', bd=bd2)
    ...
    print(data.tail())
    data = obtener_multiples_series("index", "M", bd2, *["S&P 500", "NASDAQ", "ibex 35"])
    print(data.tail())
    data = obtener_multiples_series("forex", "M", bd2, *["EUR/USD", "GBP/USD"])
    print(data.tail())
    data = obtener_multiples_series("precios", "M", bd1, *["US_AAPL", "US_AMZN", "LSE_NCC"])
    print(data.tail())
    data = obtener_multiples_series("fundamental", "Q", bd1, *["US_AAPL", "netIncome"])
    print(data.tail())
