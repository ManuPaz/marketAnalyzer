...
import utils.database.stocks_queries_string
from utils.get_new_data.get_stocks_structured_to_EOD import highlights, stocks
from utils.get_new_data.get_stocks_structured_to_EOD.datosProcesados.ratios_earnings import ratios_earnings
...
def __update_prizes_h5s(exchange, stock, bd):
    ...
    data = bd.execute_query_dataframe(
        "select fecha,stock,Adjusted_close from {}_precios where stock=%s order by fecha asc".format(exchange),
        (stock,))
    if data is not None:
...
def update_one_stock_in_all_tables(exchange, stock, bd):
    ...
    # delete from fundamental to update NULLS
    fecha_delete = dt.datetime.today() - relativedelta.relativedelta(years=6)
    query_delete = utils.database.stocks_queries_string.DELETE_SINCE_DATE_BY_STOCK.format(exchange + "_fundamental",
                                                                                          "fecha")
    # delete last 3 years
    bd.execute_manual_rollback_option(query_delete,exchange + "_fundamental", (stock, fecha_delete,))
    try:
        stocks.save_stocks_data(stock, exchange, "fundamental", bd)
    except Exception as e:
        bd.manual_rollback()
        logger.error("update_stock: {}".format(e))
