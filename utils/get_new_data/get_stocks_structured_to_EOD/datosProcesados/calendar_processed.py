...
def update_calendar_processed():
    ...
    bd.execute(query)
    countries = [e[0] for e in
                 (bd.execute_query(bd_macro_string.GET_COUNTRIES_FROM_CALENDAR.format(bd_macro_string.TABLE_CALENDAR)))]
    events = []

    start = time.time()
    for country in countries:
        if country in config["macro"]["countries_working"]:
            events_aux = [e[0] for e in (
                bd.execute_query(bd_macro_string.GET_MACRO_EVENT_BY_COUNTRY.format(bd_macro_string.TABLE_CALENDAR),
                                 (country,)))]
            ...
            while len(events) > 0:
                ...
                if save_event:
                    data = bd.execute_query_dataframe(bd_macro_string.GET_DATA_BY_EVENT_AND_ZONE_EQUALS.format(
                        bd_macro_string.TABLE_CALENDAR_PROCESSED),
                        (country, event))
                    add = data is None
                    if data is not None:
                        data["Date"] = pd.to_datetime(data["Date"])
                        if len(data) > 0 and data.loc[
                            data.index[-1], "Date"].to_pydatetime() < dt.datetime.now():
                            add = True
                    if add:
                        dic_dataframes = {}
                        for cadena in [bd_macro_string.MOM_STRING, bd_macro_string.YOY_STRING,
                        ...
                        if lenght == 0:
                            data = bd.execute_query_dataframe(
                                bd_macro_string.GET_DATA_BY_EVENT_AND_ZONE_DOUBLE_LIKE.format(
                                    bd_macro_string.TABLE_CALENDAR),
                                (country, event + "  (%", event))
                            if data is not None:
                                lenght = len(data)
                                if lenght > 0:
                        ...
                        if len(dic_dataframes) > 0:
                            ...
                            if lenght > 30:
                                ...
                                cadenas_keys.sort(key=lambda x: int(dic_dataframes[x][0].Date.iloc[
                                                                        -1].to_pydatetime() >= dt.datetime.now() - relativedelta.relativedelta(
                                ...
                                if data.loc[data.index[-1], "Date"] >= dt.datetime.now() - relativedelta.relativedelta(
                                    ...
                                    if cadena != "":
                                        if IMPROVE_PERFORMANCE:
                                            data.loc[:, "actual"] = diff_to_absolute(data.actual.values,
                                                                                     periods[cadena])
                                        else:
